  ...
  return (
    <>
      <div className="p-5 font-inter xl:p-10">
        ...
        <p className="py-5 text-lg md:text-xl" style={{color:color.textPrimary}}>
        ...
        <div className=" flex flex-row gap-4 p-4">
          <Link to="/projects">
            <p className="p-4 rounded-lg font-medium lg:px-7 lg:rounded-xl xl:rounded-none" style={{background:color.accent}}>View Projects</p>
          </Link>
          <a
            href="/src/assets/doc/Yogeth_CR_Resume.pdf"
            download="Yogeth_Resume"
            className=" border p-3 w-fit rounded-lg font-medium 
            text-lg xl:rounded-none" style={{color:color.accent}}
          >
           Download CV
          </a>
        </div>
        <p className="text-lg p-3 font-semibold tracking-wider uppercase" style={{color:color.textPrimary}}>Skills</p>
        <ul className=" grid grid-cols-3 gap-3 px-3 lg:grid-cols-4 xl:grid-cols-10 xl:gap-4 xl:pr-15">
          {HomePageData.skills.map((element, i) => (
            <li className="border p-2 flex justify-center items-center md:p-3 xl:p-4" style={{color:color.accent}} key={i}>
              <p className="text-xs">{element}</p>
            </li>
          ))}
        </ul>
         <p className="text-lg p-3 mt-4 font-semibold tracking-wider" style={{color:color.textPrimary}}>FEATURED_CAPABILITIES</p>
          ...
          {HomePageData.featuredCapabilites.map((element, i) => (
            <div key={i} className=" p-5 rounded-[7px]
             ...
             h-full w-full bg-white-900 rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-10 border border-gray-100">
              ...
              <p className="text-text-secondary  tracking-wide md:text-xl " style={{color:color.textSecondary}}>{element.para}</p>
            </div>
          ))}
      </div>
      </div>
